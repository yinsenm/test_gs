...
class PortfolioStrategy:
    ...
    def get_cov_mat(self, df_rets: pd.DataFrame):
    ...
    def _mvp_func(self, df_rets: pd.DataFrame) -> dict:
        symbols = df_rets.columns
        n_asset = len(symbols)
        self.get_cov_mat(df_rets)  # get cov mat
        if self.prev_weights is not None:
            cost = lambda wgts: self.get_port_vol(wgts, self.cov_mat) + \
        ...
        opt = minimize(cost, x0=init_weights, bounds=tuple((self.min_weight, self.max_weight) for k in range(n_asset)),
    ...
    def _mr_func(self, df_rets: pd.DataFrame) -> dict:
        ...
        if self.prev_weights is not None:
            cost = lambda wgts: - self.get_port_mu(wgts, mu_vec) + \
                                self.get_port_txn_cost(wgts, self.prev_weights)
        else:
            cost = lambda wgts: - self.get_port_mu(wgts, mu_vec)
        ...
        opt = minimize(cost, x0=init_weights, bounds=tuple((self.min_weight, self.max_weight) for k in range(n_asset)),
                       constraints=[{"type": "eq", "fun": lambda x: np.sum(x) - 1.0}])
        return {k: v for k, v in zip(symbols, set_eps_wgt_to_zeros(opt["x"]))}
    ...
    def _mvo_func(self, df_rets: pd.DataFrame) -> dict:
        ...
        opt = minimize(cost, x0=init_weights,
                       ...
                       constraints=[
                           ...
                           {"type": "ineq", "fun": lambda x: self.config["vol_target"] - self.get_port_vol(x, self.cov_mat)},
                       ], method='SLSQP')
        return {k: v for k, v in zip(symbols, set_eps_wgt_to_zeros(opt["x"]))}
    ...
    def _rp_func(self, df_rets: pd.DataFrame) -> dict:
        ...
        if self.prev_weights is not None:
            cost = lambda wgts: self.get_rp_obj(wgts, self.cov_mat) + \
                                self.get_port_txn_cost(wgts, self.prev_weights)
        else:
            cost = lambda wgts: self.get_rp_obj(wgts, self.cov_mat)
